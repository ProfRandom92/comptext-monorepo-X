...
// Reference tables — consumed by downstream triage rules and exported for documentation.
const _CRITICAL_HIGH: Record<keyof LabValues, number> = {
...
const _CRITICAL_LOW: Record<keyof LabValues, number> = {
  hs_tni: 0,
  ckmb: 0,
  pct: 0,
  crp: 0,
...
function extractLabs(kvtc: KVTCOutput): LabValues {
  ...
  for (const item of kvtc.layer_v.normalized) {
    ...
    // NIHSS is a clinical score, not a lab value — handled separately
  }
  return labs;
}
...
function extractAllergies(nurse: NURSEOutput): AllergyCode[] {
  ...
  for (const res of nurse.resources) {
    ...
    const isAllergy =
      text.toLowerCase().includes("allergie") ||
...
export function assembleFrame(
  ...
  const meta: PipelineMeta = {
    ...
    tok_out: tokOut,
    reduction_pct:
      Math.round((1 - tokOut / (bundle._meta?.tokenCountRaw ?? nurse.token_in)) * 100 * 10) / 10,
    stages: [
