...
export const PHI_PATTERNS = {
   ...
   */
  nameInText:
    /(?<=[^A-ZÄÖÜa-zäöü.!?]\s)(?!Herr|Frau|Dr|Prof|Med|Dr\.med|Prof\.Dr)[A-ZÄÖÜ][a-zäöü]{2,}(?:\s+[A-ZÄÖÜ][a-zäöü]{2,})?/g,
} as const;
...
function sanitizeString(text: string): { sanitized: string; replacements: number } {
  ...
  for (const detection of detections) {
    ...
    sanitized =
      sanitized.slice(0, detection.position) +
      placeholder +
      sanitized.slice(detection.position + detection.value.length);
    replacements++;
  }
...
function deepSanitizeStrings(
  obj: unknown,
  path: string = ""
): { result: unknown; phiFound: number } {
...
export function runNURSE(bundle: FHIRBundle): NURSEOutput {
  ...
  for (const entry of bundle.entry) {
    ...
    if (res.resourceType === "Observation") {
      ...
      const { result: sanitizedCodeText, phiFound: phiInCode } = deepSanitizeStrings(
        obs.code?.text
      );
    ...
    if (res.resourceType === "MedicationStatement") {
      ...
      const { result: sanitizedDisplay, phiFound: phiInDisplay } = deepSanitizeStrings(
        med.medicationCodeableConcept?.text
      );
      const { result: sanitizedDosage, phiFound: phiInDosage } = deepSanitizeStrings(
        med.dosage?.[0]?.text
      );
      ...
      resources.push({
        ...
        fields: {
          rxnorm: med.medicationCodeableConcept.coding?.find((c) => c.system?.includes("rxnorm"))
            ?.code,
