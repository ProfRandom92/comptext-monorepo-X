...
describe("NURSE Stage", () => {
  ...
  it("detects and hashes German ZIP codes via regex", () => {
    ...
    const observation = bundleWithPLZ.entry.find(
  ...
  it("detects and hashes German phone numbers via regex", () => {
    ...
    const observation = bundleWithPhone.entry.find(
      (e: { resource: { resourceType: string } }) => e.resource.resourceType === "Observation"
    );
    if (observation) {
  ...
  it("detects and hashes IBAN via regex", () => {
    ...
    const observation = bundleWithIBAN.entry.find(
      (e: { resource: { resourceType: string } }) => e.resource.resourceType === "Observation"
    );
    if (observation) {
  ...
  it("detects and hashes German mobile numbers via regex", () => {
    ...
    const observation = bundleWithMobile.entry.find(
      (e: { resource: { resourceType: string } }) => e.resource.resourceType === "Observation"
    );
    if (observation) {
...
describe("Full Pipeline — STEMI", () => {
  ...
  it("preserves contrast allergy in output frame", async () => {
    ...
    expect(
      algNames.some((n) => n.toLowerCase().includes("kontrast") || n.toLowerCase().includes("jod"))
    ).toBe(true);
  });
