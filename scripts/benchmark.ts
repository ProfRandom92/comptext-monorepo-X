 ...
 */
async function benchmarkScenario(scenarioId: string, bundle: FHIRBundle): Promise<BenchmarkRow> {
  ...
  await countTokens(fhirJson); // warm tokenizer / reserved for future per-row detail
...
function formatMarkdownTable(rows: BenchmarkRow[]): string {
  ...
  lines.push("|----------|----------|----------|----------|");
  lines.push(
    `| ${"AVERAGE".padEnd(8)} | ${String(avgFhirRaw).padStart(8)} | ${String(avgCompText).padStart(8)} | ${avgReduction.toFixed(1).padStart(7)}% |`
  );
  lines.push("");
...
function formatDetailedMarkdownTable(rows: BenchmarkRow[]): string {
  ...
  lines.push("|----------|----------|------------|-----------|----------|----------|");
  lines.push(
    `| ${"AVERAGE".padEnd(8)} | ${String(avgFhirRaw).padStart(8)} | ${String(avgNurse).padStart(10)} | ${String(avgKvtc).padStart(9)} | ${String(avgCompText).padStart(8)} | ${avgReduction.toFixed(1).padStart(7)}% |`
  );
  lines.push("");
...
function formatCsv(rows: BenchmarkRow[]): string {
  const header =
    "scenario,fhir_raw,post_nurse,post_kvtc,comptext,reduction_pct,reduction_abs,latency_ms";
